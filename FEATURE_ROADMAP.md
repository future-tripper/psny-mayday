...
## Phase 1: Abort/Reset Flow ✅ COMPLETE

1. [x] Add status values to Pair model (`orphaned`, `abandoned`)
2. [x] Add `last_activity_at` field to Pair model (optional, for cleaner queries) - *Used Line.created_at approach instead*
3. [x] Create `/abort` endpoint
4. [x] Create reset options page - *Created confirm_leave.html, partner_left.html*
5. [x] Update `try_pair_users()` to prioritize orphaned pairs
6. [x] Implement `cleanup_stale_pairs()` with 12-hour threshold
7. [x] Create `/session-expired` page for returning users
8. [x] Create `/rejoin` endpoint
9. [x] Test: one user aborts → other gets same slot with new partner
10. [x] Test: both users go silent 12h+ → slot freed for backfill
11. [x] Test: returning user after expiry → session expired → rejoin flow

### Phase 1 Implementation Notes:
- **Templates created**: `confirm_leave.html`, `partner_left.html`, `session_expired.html`, `goodbye.html`
- **Endpoints added**: `/confirm-leave`, `/partner-left`, `/session-expired`, `/goodbye`, `/abort`, `/restart-same-lines`, `/restart-new-lines`, `/leave-completely`, `/rejoin`
- **User statuses**: `waiting`, `paired`, `waiting_for_partner`, `inactive`
- **Pair statuses**: `writing`, `complete`, `orphaned`, `abandoned`
- **Leave collaboration link** added to writing page (index.html)
...
## Abort/Reset Flow ✅ TESTED
- [x] One user aborts → partner gets options page
- [x] Partner waits → gets new human partner in same slot
- [ ] Partner chooses AI → pairs with AI in same slot *(Phase 2)*
- [x] Both abort → slot available for new pair
- [x] Orphaned slots get filled before new slots

## Silent Abandonment (12-Hour Timeout) ✅ IMPLEMENTED
- [x] Pair with no activity for 12h+ is marked abandoned on next signup
- [x] Abandoned slot becomes available for backfill
- [x] Returning user sees "session expired" page
- [x] Returning user can rejoin waiting room
- [x] Returning user gets new slot (not their old one)
- [x] User account/code still works after expiry
